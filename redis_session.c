...
PS_OPEN_FUNC(rediscluster) {
    ...
    /* Look for password */
    if ((z_val = zend_hash_str_find(ht_conf, "password", sizeof("password") - 1)) != NULL &&
        Z_TYPE_P(z_val) == IS_STRING && Z_STRLEN_P(z_val) > 0
        ) {
    ...
    if (password && password_len > 0) {
        strncpy(c->password, password, password_len > REDIS_PASSWORD_LEN ? REDIS_PASSWORD_LEN : password_len);
    }
