...
cluster_init_seeds(redisCluster *cluster, HashTable *ht_seeds) {
    ...
    for (i = 0; i < count; i++) {
        ...
        // If have password need auth
        if (strlen(cluster->password) > 0) {
            redis_sock->auth = zend_string_init(cluster->password, strlen(cluster->password), 0);
        }
                                  ...
                                  int cmd_len TSRMLS_DC)
{
    ...
    do {
        // init redis_sock auth when cluster has valid password
        if (c->password[0] && !c->cmd_sock->auth) {
            c->cmd_sock->auth = zend_string_init(c->password, strlen(c->password), 0);
        }
