...
PHP_METHOD(RedisCluster, __construct) {
    ...
    //add password to cluster context
    if (password_len > 0 && ZEND_NUM_ARGS() >= 6) {
        strncpy(context->password, password, password_len > REDIS_PASSWORD_LEN ? REDIS_PASSWORD_LEN : password_len);
    }
